...
public class IP implements Serializable {
    ...
    private  boolean enableFileWatch = false;
    ...
    public  void load(FSDataInputStream fsdi) {
        FSDataInputStream fin = null;
        lock.lock();
        try {
            dataBuffer = ByteBuffer.allocate(fsdi.available());
//            System.out.println(fsdi.available());
            fin = fsdi;
            int readBytesLength;
...
//        System.out.println(fsdi.available());
        dataBuffer.put(bytes,0,bytes.length);
        dataBuffer.position(0);
        int indexLength = dataBuffer.getInt();
        byte[] indexBytes = new byte[indexLength];
        dataBuffer.get(indexBytes, 0, indexLength - 4);
